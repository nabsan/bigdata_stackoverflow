...
//    val pairedPost = postings.map(p =>
//       p.postingType match{
//         case 1 => (p.id, p)
//         case 2 => (p.parentId.get, p)
//       }
//    )
//    val partitioner = new RangePartitioner(8,pairedPost)
//    val questions = pairedPost.filter(_._2.postingType == 1)
//    val answers   = pairedPost.filter(_._2.postingType == 2)
...
//    questions.join(answers).groupByKey()
  ...
  def scoredPostings(grouped: RDD[(Int, Iterable[(Posting, Posting)])]): RDD[(Posting, Int)] = {
    ...
    grouped.map {case (_,iterative) => 
      (iterative.head._1, 
      answerHighScore(iterative.map(_._2).toArray))
    }
  }
